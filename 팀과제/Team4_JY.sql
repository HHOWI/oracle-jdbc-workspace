...
CREATE TABLE MATCHING_POST (    ---------------------------------  매칭글
    ...
    CATEGORY_SEQ NUMBER,
    MATCHING_POST_AUTHOR VARCHAR2(50)       -- 매칭글 작성자
);
...
ALTER TABLE CHAT_USER_INFO ADD CONSTRAINT CHATROOM_SEQ_PK PRIMARY KEY(CHATROOM_SEQ); -- 채팅참여유저정보(채팅방번호)
ALTER TABLE MATCHING_POST ADD CONSTRAINT MATCHING_POST_SEQ_PK PRIMARY KEY(MATCHING_POST_SEQ); -- 매칭글(매칭글SEQ)
ALTER TABLE MATCHING_POST_LIKE ADD CONSTRAINT MATCHING_POST_LIKE_SEQ_PK PRIMARY KEY(MATCHING_POST_LIKE_SEQ); -- 매칭글좋아요(매칭글좋아요SEQ)
...
ALTER TABLE MATCHING_POST ADD CONSTRAINT MATCHING_POST_FK2 FOREIGN KEY(MATCHING_POST_AUTHOR) REFERENCES USER_INFO(USER_ID);   -- 매칭글(작성자아디)

ALTER TABLE MATCHING_POST_LIKE ADD CONSTRAINT MATCHING_POST_LIKE_FK FOREIGN KEY(MATCHING_POST_SEQ) REFERENCES MATCHING_POST(MATCHING_POST_SEQ); -- 매칭글좋아요(매칭글SEQ)
ALTER TABLE MATCHING_POST_LIKE ADD CONSTRAINT MATCHING_POST_LIKE_FK2 FOREIGN KEY(MATCHING_POST_LIKE_USER) REFERENCES USER_INFO(USER_ID); -- 매칭글좋아요(아이디) 

ALTER TABLE CHAT_USER_INFO ADD CONSTRAINT CHAT_USER_INFO_FK1 FOREIGN KEY(CHATROOM_NUMBER) REFERENCES CHAT; -- 채팅참여유저정보(채팅방번호)
ALTER TABLE CHAT_USER_INFO ADD CONSTRAINT CHAT_USER_INFO_FK2 FOREIGN KEY(CHATUSERINFO) REFERENCES USER_INFO; -- 채팅참여유저정보(참여유저아이디)
