 /*
    ...
    >> SELECT 절에 단일행 함수와 그룹 함수는 함께 사용 불가 (결과 행의 갯수가 다르기 때문)
 ...
 */
 ...
 /*
    ...
    - 지정한 위치(POSITION)부터 지정된 숫자(OCCURRENCE) 번째로 나타나는 문자의 시작 위치를 반환
 ...
 */
 ...
 FROM DUAL;
 SELECT INSTR('AABBAACAABBAA', 'B', 1, 2)
 FROM DUAL;
 ...
 /*
    ...
    - 문자열에 덧붙이고자 하는 문자를 왼쪽 또는 오른쪽에 덧 붙여서 최종 N 길이 만큼의 문자열을 반환
 */
 ...
 */
 ...
 /*
    TRIM
    - 문자열의 앞/뒤에 있는 지정한 문자들을 제거한 나머지 문자열 반환
 ...
 */
 ...
 /*
    ...
    - SUBSTR(STRING, POSITION, [LENGHTH])
        STRING : 문자타입컬럼 또는 '문자열값'
        POSITION : 문자열을 추출할 시작위치 값 (음수값도 가능)
 ...
 */
 ...
 SELECT ROUND(123.456, -1) FROM DUAL;-- 100
 ...
 */
 ...
 /*
    TRUNC(NUMBERM, [위치]) : 위치 지정 가능한 버림처리 해주는 함수
 */
 ...
 */
 ...
 -- 직원명, 입사일, 입사월의 마지막 날짜, 입사한 월의 근무한 일수 조회
 ...
 */
 ...
 SELECT TO_CHAR(1234, 'L99') FROM DUAL; -- 포맷 자리수가 안맞아서 오류
 SELECT TO_CHAR(1234, 'L999,999') FROM DUAL; -- 
 
 -- 직원명, 급여, 연봉 (위의 TO_CHAR를 이용해서) 조회
 SELECT EMP_NAME, TO_CHAR(SALARY, '9,999,999L'), TO_CHAR(SALARY * 12, '999,999,999L') "연봉"
 FROM EMPLOYEE
 ORDER BY "연봉";
 ...
 -- DY, DAY : 요일 / DD : 일
 SELECT TO_CHAR(SYSDATE, 'MON DD"일" DY, YYYY"년"') FROM DUAL;
 ...
 -- 월과 관련된 포맷
 SELECT TO_CHAR(SYSDATE, 'MM'), TO_CHAR(SYSDATE, 'MON'), TO_CHAR(SYSDATE, 'MONTH'), TO_CHAR(SYSDATE, 'RM') -- 로마기호
 FROM DUAL;
 
 -- 일과 관련된 포맷
 SELECT
 TO_CHAR(SYSDATE, 'D'), -- 주 기준 (일, 월, 화, ...)
 TO_CHAR(SYSDATE, 'DD'), -- 월 기준
 TO_CHAR(SYSDATE, 'DDD') -- 연 기준
 FROM DUAL;
 ...
 -- 직원명, 입사일 조회 단, 입사일은 포맷을 지정해서 조회! (2023년 06월 21일 (수))
 ...
 */
 ...
 -- 사원명, 보너스포함 연봉
 SELECT EMP_NAME, TO_CHAR(NVL(((SALARY + SALARY*BONUS) * 12), (SALARY*12)), '999,999,999L') "보너스포함 연봉"
 FROM EMPLOYEE
 ORDER BY "보너스포함 연봉" DESC;
 
 -- 사원명, 부서코드 조회
 SELECT EMP_NAME "사원명", NVL(DEPT_CODE, '부서없음') "부서"
 FROM EMPLOYEE
 ORDER BY "부서", "사원명";
 
 /*
    NVL2(값1, 값2, 값3)
    - 값1이 NULL이 아니면 값2를 반환 값1이 NULL이면 값3를 반환
 */
 -- 사원명, 부서코드가 있는 경우 "부서있음", 없는 경우는 "부서없음" 조회
 SELECT EMP_NAME "사원명", NVL2(DEPT_CODE, '부서있음', '부서없음') "부서"
 FROM EMPLOYEE
 ORDER BY "부서", "사원명";
 
 /*
    NULLIF(값1, 값2)
    - 두 개의 값이 동일하면 NULL을 반환하고, 두 개의 값이 다르면 값1을 반환
 */
 ...
 /*
    ...
    DECODE(컬럼|산술연산|함수식, 조건값1, 결과값1, 조건값2, 결과값2, ..., 결과값N) 조건값이 없으면 DEFALT
 ...
 */
 ...
 -- 그외 사원은 급여를 5% 인상
 -- 정렬 : 직급 코드 J1부터, 인상된 급여가 높은 순서대로
 SELECT EMP_NAME,JOB_CODE, TO_CHAR(SALARY, '9,999,999L'), TO_CHAR(DECODE(SUBSTR(JOB_CODE, 2, 1), 7, SALARY * 1.1, 6, SALARY * 1.15, 5, SALARY * 1.2, SALARY * 1.05), '9,999,999L') "인상된 급여"
 FROM EMPLOYEE
 ...
 */
 -- 사번, 사원명, 주민번호, 성별(남, 여) 조회
 SELECT EMP_ID, EMP_NAME, EMP_NO, CASE WHEN SUBSTR(EMP_NO, 8, 1) = 1 THEN '남자' WHEN SUBSTR(EMP_NO, 8, 1) = 2 THEN '여자' ELSE '잘못된 주민번호 입니다.' END AS "성별"
 FROM EMPLOYEE;
 
 -- 사원명, 급여, 급여 등급(1~4등급)
 -- 급여 값이 500만원 초과 1등급
 -- 급여 값이 500만원 이하 350만원 초과 2등급
 -- 급여 값이 350만원 이하 200만원 초과 3등급
 -- 그 외의 경우 4등급
 SELECT EMP_NAME, SALARY, CASE WHEN SALARY > 5000000 THEN '1등급' WHEN SALARY > 3500000 THEN '2등급' WHEN SALARY > 2000000 THEN '3등급' ELSE '4등급' END "급여 등급"
 FROM EMPLOYEE
 ...
 */
 -- 전체 사원의 총 급여 합
 SELECT TO_CHAR(SUM(SALARY), 'FM999,999,999L') "총 급여"
 FROM EMPLOYEE;
 
 -- 부서코드가 D5인 사원들의 총 연봉 합
 SELECT TO_CHAR(SUM(SALARY * 12), 'FM999,999,999L') "총 연봉"
 FROM EMPLOYEE
 WHERE DEPT_CODE = 'D5';
 /*
    AVG(NUMBER)
 ...
 */
 -- 전체 사원의 평균 급여 조회
 SELECT TO_CHAR(ROUND(AVG(SALARY)), 'FM999,999,999L') "평균 급여"
 FROM EMPLOYEE;
 ...
 /*
    MIN/MAX(모든 타입의 컬럼)
    - MIN : 해당 컬럼 값들 중에 최소값 반환
    - MAX : 해당 컬럼 값들 중에 최대값 반환
 */
 ...
 /*
    ...
    - 컬럼 또는 행의 개수를 세서 반환
    COUNT(*) : 조회 결과에 해당하는 모든 행 갯수를 반환
    COUNT(컬럼) : 해당 컬럼값이 NULL이 아닌 행의 갯수를 반환
    COUNT(DISTINCT 컬럼) : 해당 컬럼값의 중복을 제거한 행의 갯수를 반환
 */
 -- 전체 사원 수
 SELECT COUNT(*)
 FROM EMPLOYEE;
