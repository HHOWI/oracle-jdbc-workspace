/*
    ...
    * PL/SQL 구조
    ...
    - [예외처리부(EXCEPTION SECTION)S : EXCEPTION으로 시작, 예외발생시 해결하기 위한 구문을 미리 기술해둘 수 있는 부분
*/
-- 출력 기능 활성화
SET SERVEROUTPUT ON;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('DITITLE : ' || DITITLE);
    DBMS_OUTPUT.PUT_LINE('SAL : ' || SAL);
END;
/
...
-- 단, 보너스를 지급 받지 않는 사원은 보너스 출력 전에 '보너스를 지급받지 않는 사원입니다.'라는 문구를 출력
...
-- 일반타입변수 TEAM을 데이터 타입 VARCHAR2(10)으로 선언하고 NCODE 값이 KO일 경우 => TEAM에 '국내팀' 대입, 그게 아닐경우 => TEAM에 '해외팀' 대입
...
-- 입력된 점수에 따라 GRADE 변수(데이터타입 : CHAR(1))에 저장
...
/*
    1-4) CASE 구문
         ...
         END CASE;    
*/
-- 사번을 입력받은 후 사원의 모든 컬럼 데이터를 EMP에 대입하고 DEPT_CODE에 따라서 알맞는 부서를 출력
-- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, D4인 경우 국내영업부, D5인 경우 해외영업1부, D6인 경우 해외영업2부
-- D7인 경우 해외영업3부, D8인 경우 기술지원부, D9인 경우 총무부, 나머지는 부서없음
-- 출력은 사번, 이름, 부서 코드, 부서가 출력되도록!
...
-- NO_DATA_FOUND : SELECT한 경과가 한 행도 없는 경우
-- TOO_MANY_ROWS : SELECT한 결과가 여러 행인 경우 (한 행이 리턴되어야 하는데..)
DECLARE 
    EID EMPLOYEE.EMP_ID%TYPE;
...
EXCEPTION
    ...
    WHEN OTHERS
    THEN DBMS_OUTPUT.PUT_LINE('에러가 발생했습니다. 관리자에 문의해 주세요.');
END;
/