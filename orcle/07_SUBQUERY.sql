 ...
 */
 ...
 -- 전지연 사원이 속해있는 부서원들의 사번, 사원명, 전화번호, 직급명, 부서명, 입사일 (단, 전지연 사원은 제외)
 ...
 */
 ...
 SELECT EMP_ID, EMP_NAME, DEPT_CODE, MANAGER_ID, '사원' AS "구분"
 FROM EMPLOYEE
 WHERE EMP_ID NOT IN(
 SELECT DISTINCT MANAGER_ID
 FROM EMPLOYEE
 ...
 SELECT EMP_ID, EMP_NAME, DEPT_CODE, CASE WHEN EMP_ID IN (
 SELECT DISTINCT MANAGER_ID
 FROM EMPLOYEE
 ...
 /*
    ...
    비교대상 < ANY(서브쿼리) : 여러개의 결과값 중에서 "한개라도" 작을 경우 (여러개의 결과값 중에서 가장 큰 값 보다 작을 경우)
 */
 -- 대리 직급임에도 과장 직급들의 최소 급여보다 많이 받는 직원들의 사번, 이름, 직급명, 급여 조회
 ...
 */
 -- 과장 직급임에도 차장 직급의 최대 급여보다 더 많이 받는 직원들의 사번, 이름, 직급이름, 급여 조회
 SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
 FROM EMPLOYEE
 JOIN JOB USING (JOB_CODE)
 ...
 */
 ...
 FROM EMPLOYEE
 WHERE EMP_NAME = '박나라')) AND EMP_NAME != '박나라';
 ...
 */
 -- 전 직원중 급여가 가장 높은 상위 5명 순위, 이름, 급여 조회
