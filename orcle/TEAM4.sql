...
CREATE TABLE POST( 
    ...
    POST_ATTACHMENT VARCHAR2(3000) NOT NULL, -- 게시글 사진,동영상 URL
    POST_NOTICE CHAR DEFAULT 'N' CHECK(POST_NOTICE IN ('Y','N')), -- 공지사항(Y/N) 
    POST_URL VARCHAR2(3000) -- 게시글 URL
...
CREATE TABLE MATCHING_REVIEW(
    ...
    REVIEW_ATTACHMENT VARCHAR2(300), -- 매칭 후기글 첨부파일
    REVIEW_DESC VARCHAR2(3000) NOT NULL, -- 매칭 후기글 내용
    REVIEW_URL VARCHAR2(300) NOT NULL -- 매칭 후기글 URL
);
...
CREATE TABLE MATCHING_POST (
    ...
    MP_ATTACHMENT VARCHAR2(3000),            -- 매칭글 파일첨부 URL
    ...
    CATEGORY_SEQ NUMBER,    -- 카테고리 SEQ
    MP_AUTHOR VARCHAR2(50),       -- 매칭글 작성자
    MP_DELETE CHAR DEFAULT 'N' NOT NULL CHECK(MP_DELETE IN ('Y', 'N')), -- 매칭글 삭제
    PLACE_SEQ VARCHAR2(300) NOT NULL  -- 매칭 지역
);
...
CREATE TABLE COMMENTS( 
    ...
    POST_SEQ NUMBER,                                                   -- 게시글 SEQ
    MATCHING_POST_SEQ NUMBER                                                      -- 매칭글 SEQ
    COMMENTS_DELETE CHAR DEFAULT 'N' NOT NULL CHECK(COMMENTS_DELETE IN ('Y', 'N'))  -- 댓글 삭제
);
...
CREATE TABLE PLACE_TYPE(
    PLACE_TYPE_SEQ NUMBER,
    PLACE_TYPE_NAME VARCHAR2(100) NOT NULL,
);
...
ALTER TABLE PLACE ADD CONSTRAINT PLACE_SEQ_PK PRIMARY KEY(PLACE_SEQ);           -- 지역 
ALTER TABLE PLACE_TYPE ADD CONSTRAINT PT_SEQ_PK PRIMARY KEY(PLACE_TYPE_SEQ);    -- 지역 분류
...
ALTER TABLE CATEGORY ADD CONSTRAINT PLACE_USER_ID_FK FOREIGN KEY(USER_ID) REFERENCES USER_INFO(USER_ID);    -- 유저아이디(유저정보, 지역)
...
ALTER TABLE CATEGORY ADD CONSTRAINT PLACE_PT_SEQ_FK FOREIGN KEY(PLACE_TYPE_SEQ) REFERENCES PLACE_TYPE(PLACE_TYPE_SEQ); -- 지역분류 SEQ(지역분류, 지역)

ALTER TABLE CATEGORY ADD CONSTRAINT PLACE_PT_SEQ_FK FOREIGN KEY(PLACE_TYPE_SEQ) REFERENCES PLACE_TYPE(PLACE_SEQ); -- 지역분류 SEQ(지역분류, 지역)
ALTER TABLE CATEGORY ADD CONSTRAINT PLACE_PT_SEQ_FK FOREIGN KEY(PLACE_TYPE_SEQ) REFERENCES PLACE_TYPE(PLACE_SEQ); -- 지역분류 SEQ(지역분류, 지역)
...
CREATE SEQUENCE SEQ_PLACE;         -- 카테고리(관심사)분류 SEQ
CREATE SEQUENCE SEQ_PLACE_TYPE;         -- 카테고리(관심사)분류 SEQ
...
DROP SEQUENCE SEQ_PLACE;         -- 카테고리(관심사)분류 SEQ
DROP SEQUENCE SEQ_PLACE_TYPE;         -- 카테고리(관심사)분류 SEQ