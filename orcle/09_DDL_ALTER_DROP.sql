...
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER; -- 오류 발생!
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(10); -- 오류 발생!
...
-- 다중 수정
-- DEPT_COPY 테이블에서 DEPT_TITLE 컬럼의 데이터타입을 VARCHAR2(40), LOCATION_ID 컬럼의 데이터 타입을 VARCHAR2(2), 
-- LNAME 컬럼의 기본값을 미국으로 변경
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(40) MODIFY LOCATION_ID VARCHAR2(2) MODIFY LNAME DEFAULT '미국';
...
-- 만약 삭제하고자 한다면
-- 방법 1. 자식테이블을 먼저 삭제한 후 부모테이블을 삭제
DROP TABLE MEMBER;
DROP TABLE MEMBER_GRADE;
