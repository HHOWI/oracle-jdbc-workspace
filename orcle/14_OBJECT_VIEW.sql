...
SELECT EMP_ID, EMP_NAME, JOB_NAME, CASE WHEN SUBSTR(EMP_NO,8,1) IN (1, 3) THEN '남' WHEN SUBSTR(EMP_NO,8,1) IN (2, 4) THEN '여' END "성별", 
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, CASE WHEN SUBSTR(EMP_NO,8,1) IN (1, 3) THEN '남' WHEN SUBSTR(EMP_NO,8,1) IN (2, 4) THEN '여' END "성별", 
        EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE) "근속년수"
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE);
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, CASE WHEN SUBSTR(EMP_NO,8,1) IN (1, 3) THEN '남' WHEN SUBSTR(EMP_NO,8,1) IN (2, 4) THEN '여' END , 
...
AS SELECT DEPT_CODE ,SUM(SALARY) 급여합계, FLOOR(AVG(NVL(SALARY, 0))) 급여평균
FROM EMPLOYEE
GROUP BY DEPT_CODE;
