 ...
 */
 ...
 -- 사원의 수, 보너스를 받는 사원의 수, 총 급여, 평균 급여, 최저 급여, 최고 급여 조회, 부서별 오름차순 정렬
 SELECT NVL(DEPT_CODE, '부서없음'), COUNT(EMP_ID) "사원의 수", COUNT(BONUS) "보너스를 받는 사원의 수", SUM(SALARY) "총 급여", ROUND(AVG(NVL(SALARY, 0))) "평균 급여", MIN(SALARY) "최저 급여", MAX(SALARY) "최고 급여"
 FROM EMPLOYEE
 GROUP BY DEPT_CODE
 ORDER BY DEPT_CODE NULLS FIRST;
 
 -- 성별 별 사원의 수 조회
 SELECT DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남', 2, '여') "성별", COUNT(*) "사원의 수"
 FROM EMPLOYEE
 ...
 */
 ...
 /*
    ...
    UNION(합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복되는 행은 제거)
    UNION ALL(합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복되는 행을 제거하지 않음)
 ...
 */
 ...
 -- 1. UNION 
 -- 부서 코드가 D5인 사원 또는 급여가 300만원 초과인 사원들의 사번, 사원명, 부서 코드, 급여를 조회
 SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
 FROM EMPLOYEE
 WHERE DEPT_CODE = 'D5'
 UNION
 ...
 -- 위 커리문 대신 HWERE 절에 OR 연산자를 사용해서 처리 가능
 SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
 FROM EMPLOYEE
 ...
 -- 위 커리문 대신 WHERE 절에 AND 연산자를 사용해서 처리 가능
 SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
 FROM EMPLOYEE
 ...
 -- 위 커리문 대신에 WHERE 절에 AND 연산자를 사용해서 처리 가능
 SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
 FROM EMPLOYEE
 WHERE DEPT_CODE = 'D5' AND SALARY <= 3000000;
