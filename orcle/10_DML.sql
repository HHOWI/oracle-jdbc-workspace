/*
    DML(Date Manipulation Language) : 데이터 조작 언어로 테이블에 값을 삽입(INSERT)하거나, 수정(UPDATE)하거나, 삭제(DELETE)하는 구문이다.
    ...
    3) INSERT INTO 테이블명 (서브쿼리);
        ...
        - 서브쿼리의 결과값이 INSERT 문에 지정된 테이블 컬럼 개수와 데이터 타입, 순서가 같아야 함
*/
DROP TABLE EMP_01;
CREATE TABLE EMP_01 (
...
-- EMP_NAME 컬럼의 NOT NULL 제약조건에 의해서 에러 발생
...
-- 표현법 3)
-- EMPLOYEE 테이블에서 직원의 사번, 이름, 부서명, 입사일을 조회해서 EMP_01 테이블 INSERT 하시오.
INSERT INTO EMP_01(
...
-- EMP_DEPT 테이블에 EMPLOYEE 테이블의 부서 코드가 D1인 직원의 사번, 이름, 부서 코드, 입사일을 삽입하고 
...
/*
    ...
    - UPDATE 시 서브쿼리를 사용해서 서브쿼리를 수행한 결과값으로 컬럼 값을 변경할 수 있다.
*/
...
UPDATE EMP_SALARY SET SALARY = 100
WHERE EMP_NAME = '노옹철';

-- EMP_SALARY 테이블에서 선동일 사장의 급여를 7000원으로, 보너스를 0.2로 변경
UPDATE EMP_SALARY SET SALARY = 7000, BONUS = 0.2
WHERE EMP_NAME = '선동일';

-- 모든 사원의 급여를 기존 급여에서 10프로 인상한 급여로 변경
...
-- DEPT_CODE 테이블에서 DEPT_ID가 D1인 부서 삭제
