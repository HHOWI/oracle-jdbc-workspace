 /*
    ...
    - 관계형(Relational) 데이터베이스
        ...
        3. 데이터의 중복을 최소화 할 수 있으며 사용하기 편리하고 데이터의 무결성, 트랜젝션 처리 등 데이터베이스 관리 시스템으로 뛰어난 성능을 보여준다.
...
/*
    ...
    - 조회하고자 하는 컴럼들은 반드시 FROM 절에 기술한 테이블에 존재하는 컬럼이어야 한다.
...
SELECT EMP_NAME, HIRE_DATE 입사일, CEIL(SYSDATE - HIRE_DATE) 근무일
FROM EMPLOYEE;
...
/*
    ...
    - 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 별칭에 포함될 경우 반드시 큰 따옴표("")로 감싸줘야 한다.
*/

-- EMPLOYEE 테이블에서 직원명, 연봉, 보너스가 포함된 연봉((급여 + (보너스 * 급여)) * 12) 조회
SELECT EMP_NAME AS 직원명, SALARY * 12 AS 연봉, (SALARY + (BONUS * SALARY)) * 12 "총 소득(원)"
FROM EMPLOYEE;
...
/*
    ...
    - 조회하고자 하는 테이블로 부터 특정 조건에 만족하는 데이터만을 조회하고자 할 때 사용
    - 이때 WHERE 절에 조건식을 제시
...
-- OR (또는), AND (그리고) 
-- 1. 부서코드가 'D6' 이거나 급여가 300만원 이상인 사원들의 사원명, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6' OR SALARY >= 3000000;

-- 2. 급여가 350만원 이상 600만원 이하를 받는 사원들의 사원명, 사번, 급여 조회
...
/*
    ...
    '%' : 0글자 이상
    비교대상컬럼 LIKE 'A%' => 비교대상컬럼값이 A로 "시작"되는걸 조회
    비교대상컬럼 LIKE '%A' => 비교대상컬럼값이 A로 "끝"나는걸 조회
    비교대상컬럼 LIKE '%A%' => 비교대상컬럼값이 A가 "포함"되는걸 조회 (키워드 검색)
    
    '_' : 1글자
    비교대상컬럼 LIKE '_A' => 비교대상컬럼값에 A앞에 무조건 한글자가 올 경우 조회
    비교대상컬럼 LIKE '__A' => 비교대상컬럼값에 A앞에 무조건 두글자가 올 경우 조회
    비교대상컬럼 LIKE '_A_' => 비교대상컬럼값에 A 앞과 뒤에 무조건 한글자씩 올 경우 조회  
...
/*
    ...
    비교대상컬럼 IN ('값1', '값2', ...)
*/
-- 부서코드가 D6, D8, D5가 아닌 부서원들의 이름, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
-- 5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스포함 연봉 조회 (별칭)
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE ((SALARY * 12) * BONUS) IS NOT NULL AND EMP_NAME LIKE '%하%';
...
/*
    ...
    FROM 테이블명
    WHERE 조건식
    ORDER BY 정렬하고자 하는 컬럼값 [ASC|DESC] [NULLS FIRST|NULLS LAST];
    
    - ASC : 오름차순 정렬 (생략시 기본값)
    - DESC : 내림차순 정렬
    - NULLS FIRST : 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞 배치 (생략시 DESC 일 때 작동)
    - NULLS LAST : 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤 배치 (생략시 ASC 일 때 작동)
*/
-- 전 사원의 사원명, 보너스 조회 (보너스 기준 오름차순 정렬)
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
-- ORDER BY BONUS;
ORDER BY BONUS DESC NULLS LAST; -- 보너스 기준 내림차순 정렬 ( NULL이 맨 뒤)