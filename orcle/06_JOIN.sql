 /*
    ...
    - 관계형 데이터베이스는 최소한의 데이터로 가각의 테이블에 담고 있음(중복을 최소화 하기 위해 최대한 쪼개서 관리)
...
/*
    2. 오라클 - 포괄 조인 / ANSI - 외부 조인 (OUTER JOIN)
        - 두 테이블 간의 JOIN시 일치하지 않는 행도 포함시켜서 조회가 가능
...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); -- 기준으로 삼고자 하는 테이블의 반대편 테이블의 컬럼 뒤에 (+) 붙이기
...
-- >> 오라클 구문 (에러 발생! 오라클 구문으로는 FULL JOIN 불가)
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE(+) = DEPT_ID(+);
...
SELECT M.EMP_ID 사원사번, M.EMP_NAME 사원명, M.DEPT_CODE 사원코드, E.EMP_ID 사수사번, E.EMP_NAME 사수명, E.DEPT_CODE 사수부서코드
FROM EMPLOYEE E, EMPLOYEE M 
WHERE M.MANAGER_ID(+) = E.EMP_ID;

SELECT E.EMP_ID 사원사번, E.EMP_NAME, E.DEPT_CODE, M.EMP_ID, M.EMP_NAME, M.DEPT_CODE
FROM EMPLOYEE E
LEFT JOIN EMPLOYEE M ON (E.MANAGER_ID = M.EMP_ID);


/*
    5. 카테시안곱(CATESIAN PRODUCT) / 교차 조인(CROSS JOIN)
    - 조인되는 모든 테이블의 각 행들이 서로서로 모두 매핑된 데이터가 검색된다. (곱집합)
    - 두 테이블의 행들이 모두 곱해진 행들의 조합이 출력 -> 방대한 데이터 출력으로 과부하 위험
*/
-- >> ANSI 구문
...
-- 2. 70년대생이면서 여자이고, 성이 전 씨인 직원들의 직원명, 주민번호, 부서명, 직급명을 조회하세요.
...
-- 3. 보너스를 받는 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역을 조회 단, 부서코드가 없는 사원도 출력
SELECT EMP_NAME, BONUS, (SALARY + (SALARY * BONUS)) * 12 연봉, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
SELECT NVL(DEPT_TITLE, '부서없음'), TO_CHAR(ROUND(AVG(SALARY)), 'FM999,999,999') "급여평균"
FROM DEPARTMENT
...
SELECT DEPT_TITLE, TO_CHAR((SUM(SALARY)), 'FM999,999,999') "급여의 합"
FROM DEPARTMENT
RIGHT JOIN EMPLOYEE ON (DEPT_ID = DEPT_CODE)
GROUP BY DEPT_TITLE
...
SELECT EMP_ID, EMP_NAME, JOB_NAME, SAL_LEVEL, DECODE(SAL_LEVEL, 'S1', '고급', 'S1', '고급', 'S2', '고급', 'S3', '중급', 'S4', '중급', 'S5', '초급', 'S6', '초급') "구분"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN SAL_GRADE ON (SALARY BETWEEN MIN_SAL AND MAX_SAL)
ORDER BY EMP_ID;
