/*
    ...
    * 오라클에서 객체(구조) : 테이블(TABLE), 뷰(VIEW), 시퀀스(SEQUENCE), 인덱스(INDEX), 패키지(PACKAGE), 트리거(TRIGGER), 프로시저(PROCEDURE),
...
/*
    ...
    * 테이블 생성
    - 테이블이란? 행(ROW)과 열(COLUMN)로 구성되는 가장 기본적인 데이터베이스 객체, 데이터베이스 내의 모든 데이터는 테이블에 저장
    ...
    * 자료형
    1. 문자(CHAR / VARCHAR2) - 반드시 크기 지정
        - CHAR : 최대 2000BYTE까지 저장 가능, 고정 길이(아무리 적은 값이 들어와도 처음 할당된 크기 그대로)
                    고정된 글자수의 데이터만이 담길 경우 사용
        - VARCHAR2 : 최대 4000BYTE까지 저장 가능, 가변 길이(담긴 값에 따라서 공간의 크기 맞춰줌)
...
INSERT INTO MEM_CHECK VALUES(2, 'user03', 'pass02', '홍길녀', '여', null, null);
...
INSERT INTO MEM_PRI VALUES(1, 'user03', 'pass02', '이순신', '남', null, null);
SELECT * FROM MEM_PRI;
...
INSERT INTO MEM VALUES (1, 'uset01', 'pass01', '홍길순', '여', null, null, null);
INSERT INTO MEM VALUES (2, 'uset02', 'pass02', '김말똥', null, null, null, 10);
INSERT INTO MEM VALUES (3, 'uset03', 'pass03', '강개순', '남', null, null, 40);
...
--> 자식테이블이 이미 사용하고 있는 값이 있을 경우 부모테이블로 부터 무조건 삭제가 안되는 "삭제제한" 옵션이 걸려있음
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지
    - ON DELETE RESTRICTED(기본값) : 삭제제한옵션으로, 자식데이터로 쓰이는 부모데이터는 삭제 안되게끔
...
FROM EMPLOYEE
WHERE 1 = 0;  -- 구조만 복사하고자 할 때 사용, 데이터는 복사되지 않음
